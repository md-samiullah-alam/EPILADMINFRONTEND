...
// PDF imports - Agar error aaye to comment karo
...
export default function AdminWorkList() {
  ...
  const downloadSamplePDF = () => {
    try {
      ...
      autoTable(doc, {
        ...
        body: [
          ["Daily Sales", "Daily", "60M", "", "", "https://docs.google.com/..."],
          ["Weekly Meeting", "Weekly", "90M", "Monday,Wednesday,Friday", "", "Meeting agenda"],
  ...
  const downloadPDFReport = () => {
    if (!worklists.length) return toast.info("No data");
    try {
      const doc = new jsPDF();
      doc.setFontSize(16);
  ...
  return (
    <div className="p-4 sm:p-6 max-w-7xl mx-auto">
      ...
      <div className="bg-white rounded-xl border p-4 mb-6 flex flex-wrap gap-4">
        ...
        <input type="text" placeholder="🔍 Search..." value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} className="border rounded-lg px-3 py-2 text-sm flex-1" />
      ...
      : worklists.length === 0 ? <div className="text-center py-20 bg-white rounded-xl border-2 border-dashed"><p>No worklists found</p></div>
      : (
        <>
          <div className="bg-white rounded-xl border overflow-x-auto">
            <table className="w-full text-sm">
              <thead><tr className="bg-slate-50 text-xs uppercase">
                <th className="p-3">#</th><th className="p-3">Employee</th><th className="p-3">Worklist</th>
                <th className="p-3">Frequency</th><th className="p-3">Schedule</th><th className="p-3">Time</th>
                <th className="p-3">Link/Remark</th><th className="p-3">Actions</th>
               </tr></thead>
              <tbody>
                {paginatedData.map((wl, idx) => (
                  <tr key={wl.WorkListId} className="border-t hover:bg-slate-50">
                    ...
                    <td className="p-3 max-w-[200px] text-xs">{wl.TemplateLink ? <a href={wl.TemplateLink} target="_blank" className="text-blue-600">📎 Link</a> : (wl.Remark || "-")}</td>
                    <td className="p-3 text-center"><button onClick={() => openEdit(wl)} className="bg-blue-600 text-white px-3 py-1 rounded text-xs">Edit</button></td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
          {totalPages > 1 && <div className="flex justify-center gap-4 mt-6"><button disabled={page===1} onClick={()=>setPage(p=>p-1)} className="px-4 py-2 bg-slate-200 rounded-lg">Prev</button><span>Page {page} of {totalPages}</span><button disabled={page===totalPages} onClick={()=>setPage(p=>p+1)} className="px-4 py-2 bg-slate-200 rounded-lg">Next</button></div>}
        </>
      )}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50 p-4" onClick={(e)=>handleClickOutside(e,addModalRef,closeModal)}>
          <div ref={addModalRef} className="bg-white rounded-xl w-full max-w-md p-6 max-h-[90vh] overflow-y-auto relative">
            ...
            <div className="space-y-4">
              ...
              <div><label className="block text-xs font-bold mb-1">Frequency *</label>
              ...
              <div><label className="block text-xs font-bold mb-1">Working Time (Minutes) *</label>
                <select value={WORKING_TIMES.includes(form.WorkingTime)?form.WorkingTime:"custom"} onChange={e=>handleWorkingTimeChange(e.target.value)} className="w-full border rounded-lg px-3 py-2 text-sm mb-2">
                  <option value="30M">30 Min</option><option value="60M">60 Min (1 Hour)</option><option value="90M">90 Min (1.5 Hours)</option>
                  <option value="120M">120 Min (2 Hours)</option><option value="180M">180 Min (3 Hours)</option><option value="240M">240 Min (4 Hours)</option>
                  <option value="300M">300 Min (5 Hours)</option><option value="custom">Custom...</option>
                </select>
                {(!WORKING_TIMES.includes(form.WorkingTime) || form.WorkingTime==="custom") && <input type="number" placeholder="Enter minutes" value={customWorkingTime} onChange={e=>{setCustomWorkingTime(e.target.value); if(e.target.value) setForm({...form,WorkingTime:`${parseInt(e.target.value)}M`});}} className="w-full border rounded-lg px-3 py-2 text-sm"/>}
              </div>
              
              {!editing && (<div><label className="block text-xs font-bold mb-1">Employee *</label><select value={form.EmployeeName} onChange={e=>setForm({...form,EmployeeName:e.target.value})} className="w-full border rounded-lg px-3 py-2 text-sm"><option value="">Select</option>{employees.map(e=><option key={e.name} value={e.name}>{e.name}</option>)}</select></div>)}
              
              <div><label className="block text-xs font-bold mb-1">Template Link / Remark</label><textarea value={form.TemplateLinkRemark} onChange={e=>setForm({...form,TemplateLinkRemark:e.target.value})} rows="3" className="w-full border rounded-lg px-3 py-2 text-sm" placeholder="Paste Google Sheet link OR add remark..."/><p className="text-xs text-slate-400">💡 http:// = Template Link, otherwise = Remark</p></div>
            </div>
            <div className="flex justify-end gap-3 mt-6"><button onClick={closeModal} className="px-4 py-2 bg-gray-200 rounded-lg">Cancel</button><button onClick={handleSave} disabled={saving} className="px-4 py-2 bg-blue-600 text-white rounded-lg">{saving?"Saving...":"Save"}</button></div>
          </div>
        </div>
      )}
      ...
      {showBulk && (
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50 p-4" onClick={(e)=>handleClickOutside(e,bulkModalRef,closeBulkModal)}>
          <div ref={bulkModalRef} className="bg-white rounded-xl w-full max-w-lg p-6 max-h-[85vh] overflow-y-auto relative">
            <button onClick={closeBulkModal} className="absolute top-4 right-4 text-gray-400 text-xl">✕</button>
            <h2 className="text-lg font-black mb-4">📤 Bulk Upload</h2>
            {selectedEmp==="all" ? <div className="p-4 bg-amber-50 rounded-lg text-amber-700">⚠️ Select employee first</div>
            : (<>
              <div className="mb-4 p-3 bg-blue-50 rounded-lg">👤 Uploading for: <strong>{selectedEmp}</strong></div>
              <div className="mb-4 p-4 bg-amber-50 border-2 border-amber-300 rounded-lg">
                <p className="font-bold text-amber-800 mb-2">📋 Sample Format</p>
                <div className="flex gap-3 mb-3">
                  <button onClick={downloadSampleBulkUpload} className="bg-emerald-600 text-white px-4 py-2 rounded-lg flex-1">📥 Excel Sample</button>
                  <button onClick={downloadSamplePDF} className="bg-red-600 text-white px-4 py-2 rounded-lg flex-1">📄 PDF Sample</button>
                </div>
                <div className="text-xs">Columns: WorklistName, Frequency, WorkingTime, ScheduleDays, ScheduleDates, TemplateLinkRemark</div>
              </div>
              ...
              {bulkData.length>0 && <div className="mb-4"><p>{bulkData.length} rows ready</p><div className="max-h-40 overflow-y-auto text-xs border rounded-lg"><table className="w-full"><tbody>{bulkData.slice(0,5).map((d,i)=><tr key={i} className="border-t"><td className="p-2">{d.WorklistName}</td><td className="p-2">{d.Frequency}</td><td className="p-2">{d.WorkingTime}</td></tr>)}</tbody></table></div></div>}
              {bulkResult && <div className="mb-4 p-3 bg-slate-50 rounded-lg"><p className="text-emerald-600">✅ Created: {bulkResult.summary?.created||0}</p><p className="text-amber-600">⏭️ Skipped: {bulkResult.summary?.skipped||0}</p><p className="text-rose-600">❌ Errors: {bulkResult.summary?.errors||0}</p></div>}
              <div className="flex justify-end gap-3"><button onClick={closeBulkModal} className="px-4 py-2 bg-gray-200 rounded-lg">Close</button><button onClick={handleBulkUpload} disabled={uploading||!bulkData.length} className="px-4 py-2 bg-indigo-600 text-white rounded-lg">{uploading?"Uploading...":`Upload ${bulkData.length} rows`}</button></div>
            </>)}
          </div>
        </div>
      )}
